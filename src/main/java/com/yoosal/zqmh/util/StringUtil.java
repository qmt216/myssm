...
public class StringUtil {
    ...
    public static boolean compareVc(HttpServletRequest request, String paraName, Object value) {
        if (StringUtil.isEmpty(request.getParameter(paraName))) {
            return false;
        }
        if (value instanceof Integer) {
            return StringUtil.toInt(request.getParameter(paraName)) > (Integer) value;
        }
        if (value instanceof String) {
            return StringUtil.convertNull(request.getParameter(paraName), "").compareTo(value.toString()) > 0;
        }
        if (value instanceof Float) {
            return StringUtil.toFloat(request.getParameter(paraName)) > (Float) value;
        }
        return false;
    }
    ...
    public static void main(String[] args){
         System.out.println(StringUtil.decrypt("oLChjv5SV97c7k7y0BaIRg=="));
    }
}
